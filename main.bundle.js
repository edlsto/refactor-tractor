...
let domUpdates = {
  ...
  updateRecipesToCook(target, user) {
    ...
    if (!jquery__WEBPACK_IMPORTED_MODULE_1___default()(target).hasClass('add-active')) {
      jquery__WEBPACK_IMPORTED_MODULE_1___default()(target).addClass('add-active');
      user.addRecipesToCook(specificRecipe);
    } else if (jquery__WEBPACK_IMPORTED_MODULE_1___default()(target).hasClass('add-active')) {
      jquery__WEBPACK_IMPORTED_MODULE_1___default()(target).removeClass('add-active');
  ...
  getRecipesToCook(user) {
    if (user.recipesToCook.length) {
      user.recipesToCook.forEach(recipe => {
        jquery__WEBPACK_IMPORTED_MODULE_1___default()(`.add${recipe.id}`).addClass('add-active')
      })
    } else {
      return
    }
  ...
  getFavorites(user) {
    if (user.favoriteRecipes.length) {
      user.favoriteRecipes.forEach(recipe => {
        jquery__WEBPACK_IMPORTED_MODULE_1___default()(`.favorite${recipe.id}`).addClass('favorite-active')
      })
    } else {
      return
    }
  ...
  populateCards(user, recipes = user.cookbook.recipes) {
    let cardArea = jquery__WEBPACK_IMPORTED_MODULE_1___default()('.card-section');
    cardArea.html('');
  ...
  greetUser(user) {
    const userName = jquery__WEBPACK_IMPORTED_MODULE_1___default()('.user-name');
    userName.html(user.name.split(' ')[0] + ' ' + user.name.split(' ')[1][0]);
  },

  searchByName(user) {
    let headerSearch = jquery__WEBPACK_IMPORTED_MODULE_1___default()('#search-input');
    let cardArea = jquery__WEBPACK_IMPORTED_MODULE_1___default()('.all-cards');
  ...
  closeRecipe(user) {
    let cardArea = jquery__WEBPACK_IMPORTED_MODULE_1___default()('.display-recipe');
    cardArea.removeClass('display-recipe');
    cardArea.addClass('all-cards');
    this.populateCards(user);
  },
  ...
  displayDirections(event, user) {
    ...
    let recipeObject = new _recipe__WEBPACK_IMPORTED_MODULE_0__["default"](newRecipeInfo, user.pantry.ingredientsData);
   ...
   <button id= aria-label='delete-button' class='delete-button'> 'cook-recipe' </button>
    ...
    `);
    let alert = jquery__WEBPACK_IMPORTED_MODULE_1___default()('.alert')
    ...
    if (!user.pantry.canCookMeal(newRecipeInfo)) {
      ...
      user.pantry.getItemsNeeded(newRecipeInfo).forEach(item => {
        jquery__WEBPACK_IMPORTED_MODULE_1___default()('.alert').append(`<div class="shopping-list"><p>${upperCase(item.name)} (${Math.round(item.amountNeeded * 100) / 100} ${item.unit}, at a cost of $${(item.costOfWhatsNeededInCents / 100).toFixed(2)})</p></div>`)
      });
    }
    let ingredientsSpan = jquery__WEBPACK_IMPORTED_MODULE_1___default()('.ingredients');
    let instructionsSpan = jquery__WEBPACK_IMPORTED_MODULE_1___default()('.instructions');
  ...
  favoriteCard(event, user) {
    let favButton = jquery__WEBPACK_IMPORTED_MODULE_1___default()('.view-favorites');
    let specificRecipe = user.cookbook.recipes.find(recipe => {
      if (recipe.id  === Number(event.target.id)) {
        return recipe;
      }
    })
    if (!jquery__WEBPACK_IMPORTED_MODULE_1___default()(event.target).hasClass('favorite-active')) {
      jquery__WEBPACK_IMPORTED_MODULE_1___default()(event.target).addClass('favorite-active');
      favButton.html('View Favorites');
      user.addToFavorites(specificRecipe);
    } else if (jquery__WEBPACK_IMPORTED_MODULE_1___default()(event.target).hasClass('favorite-active')) {
      jquery__WEBPACK_IMPORTED_MODULE_1___default()(event.target).removeClass('favorite-active');
  ...
  cardButtonConditionals(event, user) {
    if (this.checkKeyboardEvent(event) === true) {
      if (jquery__WEBPACK_IMPORTED_MODULE_1___default()(event.target).hasClass('favorite')) {
        this.favoriteCard(event, user);
      } else if (jquery__WEBPACK_IMPORTED_MODULE_1___default()(event.target).hasClass('card-picture')) {
        this.displayDirections(event, user);
      } else if (jquery__WEBPACK_IMPORTED_MODULE_1___default()(event.target).hasClass('add')) {
        this.updateRecipesToCook(event.target, user);
      } else if (jquery__WEBPACK_IMPORTED_MODULE_1___default()(event.target).hasClass('add-button')) {
        this.updateRecipesToCook(jquery__WEBPACK_IMPORTED_MODULE_1___default()(event.target).children()[0], user);
      }
    }
  },
  ...
  viewRecipesToCook(event, user) {
    let viewToCookButton = jquery__WEBPACK_IMPORTED_MODULE_1___default()('#view-to-cook-button');
    let cardArea = jquery__WEBPACK_IMPORTED_MODULE_1___default()('.all-cards');
    if (cardArea.hasClass('all')) {
      cardArea.removeClass('all')
    }
...
let favButton = jquery__WEBPACK_IMPORTED_MODULE_1___default()('.view-favorites');
let homeButton = jquery__WEBPACK_IMPORTED_MODULE_1___default()('.home')
let cardArea = jquery__WEBPACK_IMPORTED_MODULE_1___default()('.all-cards');
let headerSearch = jquery__WEBPACK_IMPORTED_MODULE_1___default()('#search-input');
let viewToCookButton = jquery__WEBPACK_IMPORTED_MODULE_1___default()('#view-to-cook-button');
...
let checklist = jquery__WEBPACK_IMPORTED_MODULE_1___default()('#checklist')
...
function onStartup(recipes, ingredients, users) {
  ...
  const user = new _user__WEBPACK_IMPORTED_MODULE_2__["default"](userId, newUser.name, newUser.pantry, recipes, ingredients)
  user.populateCards();
  _domUpdates__WEBPACK_IMPORTED_MODULE_3__["default"].greetUser(user);

  //Event listeners
  headerSearch.on('keyup', () => user.searchByName())
  cardArea.on('click keypress', () => _domUpdates__WEBPACK_IMPORTED_MODULE_3__["default"].cardButtonConditionals(event, user));
  homeButton.on('click', () => {
    favButton.html('View Favorites');
    user.populateCards()
  })
  ...
  checklist.on('click', () => {
    let selected = [];
    jquery__WEBPACK_IMPORTED_MODULE_1___default()('#checklist input:checked').each(function() {
      selected.push(jquery__WEBPACK_IMPORTED_MODULE_1___default()(this).attr('name'));
    });
    user.filterRecipes(selected)
  })
  cardArea.on('click', () => {
    if (jquery__WEBPACK_IMPORTED_MODULE_1___default()(event.target).hasClass('close-btn')) {
      _domUpdates__WEBPACK_IMPORTED_MODULE_3__["default"].closeRecipe(user)
    }
    if (jquery__WEBPACK_IMPORTED_MODULE_1___default()(event.target).hasClass('delete-button')) {
      const recipeId = jquery__WEBPACK_IMPORTED_MODULE_1___default()(event.target).parent().prev().prev()[0].id;
    ...
    if (jquery__WEBPACK_IMPORTED_MODULE_1___default()(event.target).hasClass('post-button')) {
      const recipeId = jquery__WEBPACK_IMPORTED_MODULE_1___default()(event.target).parent().prev().prev()[0].id;
      const currentRecipe = user.cookbook.recipes.find(recipe => {
        return recipe.id === Number(recipeId);
      })
...
class User {
  constructor(id, name, pantry, recipes, ingredients) {
    this.id = id;
    this.name = name;
    this.pantry = new _pantry__WEBPACK_IMPORTED_MODULE_0__["default"](pantry, ingredients.ingredientsData);
    this.favoriteRecipes = [];
    this.recipesToCook = [];
    this.cookbook = new _cookbook__WEBPACK_IMPORTED_MODULE_1__["default"](recipes.recipeData);
    this.cookbook.recipes = this.cookbook.recipes.map((recipe) => {
      return new _recipe__WEBPACK_IMPORTED_MODULE_2__["default"](recipe, ingredients.ingredientsData)
    })
  }
  ...
  populateCards() {
   _domUpdates__WEBPACK_IMPORTED_MODULE_3__["default"].populateCards(this)
  }

  searchByName() {
   _domUpdates__WEBPACK_IMPORTED_MODULE_3__["default"].searchByName(this)
  }

  viewFavorites() {
    _domUpdates__WEBPACK_IMPORTED_MODULE_3__["default"].viewFavorites(this)
  }

  filterRecipes(selected) {
    _domUpdates__WEBPACK_IMPORTED_MODULE_3__["default"].filterRecipes(this, selected)
  }

  viewRecipesToCook(event) {
    _domUpdates__WEBPACK_IMPORTED_MODULE_3__["default"].viewRecipesToCook(event, this)
  }
